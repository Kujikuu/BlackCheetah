...
<template>
    <VDialog v-model="dialogValue" max-width="800">
        <VCard v-if="editedNote">
            ...
            <VCardText>
                <VForm @submit.prevent="onSubmit">
                    <VRow>
                        <VCol cols="12">
                            <AppTextField v-model="editedNote.title" label="Title" placeholder="Enter note title"
                                :disabled="isSubmitting" />
                        </VCol>

                        <VCol cols="12">
                            <AppTextarea v-model="editedNote.description" label="Description"
                                placeholder="Enter note description..." rows="6" :disabled="isSubmitting" />
                        </VCol>

                        <!-- Existing Attachments -->
                        <VCol v-if="editedNote.attachments && editedNote.attachments.length > 0" cols="12">
                            <h6 class="text-h6 mb-3">
                                Existing Attachments
                            </h6>
                            <VRow>
                                <VCol v-for="(attachment, index) in editedNote.attachments" :key="index" cols="12"
                                    md="6">
                                    <VCard variant="tonal" class="d-flex align-center pa-3">
                                        <VIcon :icon="getFileIcon(attachment.type)" size="32" class="me-3" />
                                        ...
                                        <VBtn icon size="small" variant="text" color="error" :disabled="isSubmitting"
                        ...
                        <!-- New Attachments -->
                        <VCol cols="12">
                            <VFileInput label="Attachments" multiple prepend-icon="tabler-paperclip"
                                placeholder="Upload files" chips show-size :disabled="isSubmitting"
            ...
            <VCardActions>
                ...
                <VBtn color="primary" :loading="isSubmitting" :disabled="isSubmitting" @click="onSubmit">
                    Update Note
                </VBtn>
            </VCardActions>
        </VCard>
