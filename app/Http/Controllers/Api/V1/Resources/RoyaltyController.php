...
class RoyaltyController extends BaseResourceController
{
    ...
    public function statistics(Request $request): JsonResponse
    {
        ...
        // Base query without period filter for all-time stats
        $baseQuery = Royalty::whereIn('franchise_id', $franchiseIds);

        // Apply additional filters to base query
        if ($request->has('franchise_id')) {
            $baseQuery->byFranchise($request->franchise_id);
        }

        if ($request->has('unit_id')) {
            $baseQuery->byUnit($request->unit_id);
        }

        // Query with period filter for period-specific stats
        $query = clone $baseQuery;

        // Apply period filter based on billing period (period_year and period_month)
        if ($request->has('period') && $request->period !== 'all') {
            if ($request->period === 'daily') {
                // For daily, show royalties due today
                $query->whereDate('due_date', now()->toDateString());
            } elseif ($request->period === 'monthly') {
                // For monthly, show this month's billing period
                $query->where('period_month', now()->month)
                    ->where('period_year', now()->year);
            } elseif ($request->period === 'yearly') {
                // For yearly, show this year's billing periods
                $query->where('period_year', now()->year);
            }
        }

        // Apply additional year filter if specified
        if ($request->has('year') && !$request->has('period')) {
            $query->where('period_year', $request->year);
        }

        $stats = [
            // All-time totals (not affected by period filter)
            'royalty_collected_till_date' => (float) (clone $baseQuery)->paid()->sum('total_amount'),
            // All upcoming/pending royalties (not filtered by period)
            'upcoming_royalties' => (float) (clone $baseQuery)->pending()->sum('total_amount'),
            ...
            'pending_amount' => (float) (clone $query)->pending()->sum('total_amount'),
            'paid_royalties' => (clone $query)->paid()->count(),
            'paid_amount' => (float) (clone $query)->paid()->sum('total_amount'),
            'overdue_royalties' => (clone $query)->overdue()->count(),
            'overdue_amount' => (float) (clone $query)->overdue()->sum('total_amount'),
            ...
            'monthly_royalties' => (clone $query)->where('type', 'royalty')
                ...
                ->limit(12)
                ->get(),
            'average_royalty_amount' => (float) ((clone $query)->avg('total_amount') ?? 0),
    ...
    public function myUnitRoyaltyStatistics(Request $request): JsonResponse
    {
        ...
        // Base query without period filter for all-time stats
        $baseQuery = Royalty::where('unit_id', $unit->id);
        ...
        if ($request->has('period') && $request->period !== 'all') {
            if ($request->period === 'daily') {
                // For daily, show today's due royalties
                $query->whereDate('due_date', now()->toDateString());
            } elseif ($request->period === 'monthly') {
                // For monthly, show this month's billing period
                $query->where('period_month', now()->month)
